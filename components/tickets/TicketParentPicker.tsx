...
import { TICKET_TYPE_META } from "@/lib/tickets/metadata";
import {
...
/**
 * Sélecteur de parent générique selon le type d'enfant.
 * Appelle listPotentialParentsAction qui retourne uniquement les parents
 * valides pour le childType (selon ALLOWED_PARENTS).
 */
export function TicketParentPicker({
  projectId,
  childType,
  value,
  onChange,
  ...
  // Reset si childType change (ex: l'utilisateur change le type dans le dialogue)
  useEffect(() => {
    setSelected(null);
    setResults([]);
    setQuery("");
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const res = await listPotentialParentsAction({
        projectId,
        childType,
  ...
  }, [open, query, projectId, childType]);
            ...
            {!loading &&
              results.map((p) => (
                <li key={p.id}>
                  ...
                  >
                    <ParentIcon type={p.type} />
                    <span className="font-mono text-xs text-muted-foreground shrink-0">{p.key}</span>
...
function ParentIcon({ type }: { type: TicketType }) {
  const meta = TICKET_TYPE_META[type];
  const Icon = meta.icon;
  return <Icon className={cn("h-3.5 w-3.5 shrink-0", meta.iconColor)} aria-hidden />;
}
